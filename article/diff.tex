...
%DIF ADD article/article_r1.tex   Fri Aug 29 16:29:10 2025
...
\par \DIFaddend Despite its importance, detectability is frequently unaccounted for in studies \DIFdelbegin \DIFdel{; and for }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bennett_how_2024}}\hskip0pt%DIFAUXCMD
. 
For }\DIFaddend many species, the ancillary data needed to estimate detectability do not exist\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{bennett_how_2024}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{, and so detectability is simply not used \mbox{%DIFAUXCMD
\citep{bennett_how_2024}}\hskip0pt%DIFAUXCMD
.
For other species, if the species is rare or occurs in difficult to sample areas, detections will naturally be low.
This is particularly true for threatened species in Canada and the US, whereby a very low proportion of threatened species have any detectability estimates associated with them \mbox{%DIFAUXCMD
\citep{bennett_how_2024}}\hskip0pt%DIFAUXCMD
.
When considering how to allocate conservation budgets, decision science tools can be used \mbox{%DIFAUXCMD
\citep{canessa_when_2015, binley_minimizing_2023}}\hskip0pt%DIFAUXCMD
, but only if there are reasonable estimates of detection probability for the species \mbox{%DIFAUXCMD
\citep{bennett_how_2024}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

\par \DIFdelbegin \DIFdel{However, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend wealth of openly-available observation data for birds in North America provides the opportunity to estimate detectability for a large number of species \citep{bennett_how_2024}. 
...
The NA-POPS }\DIFaddend project expanded previous efforts by the Boreal Avian Modelling project (\citet{cumming_toward_2010}, see also https://borealbirds.ca) to \DIFaddbegin \DIFadd{harmonize data \mbox{%DIFAUXCMD
\citep{barker_ecological_2015} }\hskip0pt%DIFAUXCMD
and }\DIFaddend systematically estimate detectability for landbirds \citep{solymos_calibrating_2013, solymos_evaluating_2018}.

\par Both the NA-POPS project and previous estimates by Boreal Avian Modelling project use the QPAD approach \citep{solymos_calibrating_2013}, which is a flexible method to estimate availability and perceptibility given a harmonized dataset \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{barker_ecological_2015} }\hskip0pt%DIFAUXCMD
}\DIFaddend with the necessary survey-specific data (i.e., distance to bird from observer and time to first detection; \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{barker_ecological_2015}}\hskip0pt%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Several assumptions are made for removal sampling and distance sampling, most notably that the bird is recorded as soon as it is first detected, that the bird is recorded at the location/distance it is first detected, and that subsequent movements by the bird are not recorded as new detections \mbox{%DIFAUXCMD
\citep{buckland_distance_2015, farnsworth_removal_2002, alldredge_time--detection_2007}}\hskip0pt%DIFAUXCMD
.
...
As previously mentioned, many factors can affect these metrics, such as survey time of day, survey time of year, vegetation structure, and nearby roads \mbox{%DIFAUXCMD
\citep{edwards_point_2023}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

\DIFaddbegin \subsection{\DIFadd{The QPAD Approach to Estimating Detectability}}
\DIFaddend \par \DIFaddbegin \DIFadd{Here, and in the following subsections, we review the QPAD models as derived in \mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013}}\hskip0pt%DIFAUXCMD
; we describe them here rather than in the Methods section as we feel it is important to have context for the models that we build upon in this paper.
}

\subsubsection{\DIFadd{Point Count Data}}
\par \DIFadd{We will first begin by describing the point count data that are to be used in the models.
...
, then these recorded observed abundances will be further subdivided into a time band $j = 1,...,J_i$, where $J_i$ is the maximum number of time bins for sampling event $i$.
...
, then these recorded observed abundances will also be further subdivided into a distance bin $k = 1,...,K_i$, where $K_i$ is the maximum number of distance bins for sampling event $i$.
...
The data for these species fall short of the \DIFaddbegin \DIFadd{recommended }\DIFaddend threshold of 75 distinct observations \DIFdelbegin \DIFdel{recommended }\DIFdelend \DIFaddbegin \DIFadd{(that is, 75 distinct observations in a time bins or 75 distinct observations in a distance bin) }\DIFaddend for removal and distance modelling \citep{buckland_introduction_2001, solymos_calibrating_2013}. 
...
Species with small population sizes and rare habitats are also often considered vulnerable \citep{davies_synergistic_2004, gray_effects_1989, segura_specialist_2007}, and thus detectability information would be particularly important to inform their conservation \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{because there would be better opportunity to build more informed decision models for allocating conservation budgets of monitoring versus action for these vulnerable species \mbox{%DIFAUXCMD
\citep{bennett_when_2018,bennett_how_2024}
}\hskip0pt%DIFAUXCMD
}\DIFaddend 
...
\DIFaddbegin \DIFadd{In essence, a multi-species model of the components of detectability can be created by sharing information on the observational process across species.
This differs from previous multi-species models which share information across the ecological process that determine abundance \mbox{%DIFAUXCMD
\citep{gilbert_multispecies_2024}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
\subsubsection{\DIFdel{Point Count Data}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \par %%%
\DIFdel{We used the same standardized database of point count data that was assembled through the NA-POPS project \mbox{%DIFAUXCMD
\cite{edwards_point_2023}}\hskip0pt%DIFAUXCMD
. 
This consists of $712,138$ point counts conducted across $292$ projects in Canada and the United States, $422,514$ of which used removal sampling \mbox{%DIFAUXCMD
\citep{alldredge_time--detection_2007, farnsworth_removal_2002} }\hskip0pt%DIFAUXCMD
and $522,820$ of which used distance sampling \mbox{%DIFAUXCMD
\citep{buckland_introduction_2001, buckland_distance_2015}}\hskip0pt%DIFAUXCMD
.
 All point counts were recorded in the database using a standard method developed by the Boreal Avian Modelling project \mbox{%DIFAUXCMD
\citep{barker_ecological_2015}}\hskip0pt%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsubsection{\DIFdel{Phylogenetic Trees}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \par %%%
\DIFdel{We accessed 2000 pseudo-posterior trees from \mbox{%DIFAUXCMD
\citet{jetz_global_2012} }\hskip0pt%DIFAUXCMD
for all species with sufficient data for removal modelling ($>75$ observations as per \mbox{%DIFAUXCMD
\cite{edwards_point_2023,solymos_evaluating_2018,buckland_introduction_2001}}\hskip0pt%DIFAUXCMD
). 
...
%DIFDELCMD < \par %%%
\DIFdel{We generated a phylogenetic correlation matrix $\mathbf{C}$ using the same methods outlined in the Appendix of \mbox{%DIFAUXCMD
\citet{solymos_phylogeny_2018} }\hskip0pt%DIFAUXCMD
that generate distances between species based on covariance, as processed using the R package "ape" \mbox{%DIFAUXCMD
\citep{paradis_ape_2019}}\hskip0pt%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsubsection{\DIFdel{Species Traits}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \par %%%
\DIFdel{Species traits (song pitch, migratory vs resident, open vs. closed habitat) were obtained from Birds of the World accounts for all species considered in this study \mbox{%DIFAUXCMD
\citep{billerman_birds_2022}}\hskip0pt%DIFAUXCMD
. 
...
Body mass for each bird species was obtained from the Elton traits dataset if available \mbox{%DIFAUXCMD
\citep{wilman_eltontraits_2014}}\hskip0pt%DIFAUXCMD
, and through the Birds of the World account \mbox{%DIFAUXCMD
\citep{billerman_birds_2022} }\hskip0pt%DIFAUXCMD
if not.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Bayesian Hierarchical Models}
...
, but estimated in a Bayesian rather than maximum likelihood framework}\DIFdelend \DIFaddbegin \DIFadd{$\log \phi_s \sim N(0,1)$, and $\log \tau_s \sim N(0,1)$, implying that the values of $\phi$ and $\tau$ are drawn as independent samples.
}

\subsection{\DIFadd{Data Collection}}
\subsubsection{\DIFadd{Point Count Data}}
\par \DIFadd{We used the same standardized database of point count data that was assembled through the NA-POPS project \mbox{%DIFAUXCMD
\citep{edwards_point_2023}}\hskip0pt%DIFAUXCMD
. 
This consists of $712,138$ point counts conducted across $292$ projects in Canada and the United States.
Fifty-nine percent of these point counts used removal sampling \mbox{%DIFAUXCMD
\citep{alldredge_time--detection_2007, farnsworth_removal_2002} }\hskip0pt%DIFAUXCMD
and 73\% of these point counts used distance sampling \mbox{%DIFAUXCMD
\citep{buckland_introduction_2001, buckland_distance_2015}}\hskip0pt%DIFAUXCMD
, noting that point counts could employ both types of sampling simultaneously as described previously.
All point counts were recorded in the database using a standard method developed by the Boreal Avian Modelling project \mbox{%DIFAUXCMD
\citep{barker_ecological_2015}}\hskip0pt%DIFAUXCMD
}\DIFaddend .

\DIFdelbegin \subsection{\DIFdel{Model Assessment}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \subsubsection{\DIFdelbegin \DIFdel{Objective 1: Single-Species vs. Multi-Species Estimates}\DIFdelend \DIFaddbegin \DIFadd{Phylogenetic Trees}\DIFaddend }
\par \DIFdelbegin \DIFdel{To test our prediction that there would be little change between estimates of }\DIFdelend \DIFaddbegin \DIFadd{We accessed 2000 pseudo-posterior trees from \mbox{%DIFAUXCMD
\citet{jetz_global_2012} }\hskip0pt%DIFAUXCMD
for all }\DIFaddend species with sufficient \DIFdelbegin \DIFdel{sizes, we ran the Bayesian hierarchical QPAD Removal and Distance models on the set of species for which there was sufficient data.
}\DIFdelend \DIFaddbegin \DIFadd{data for removal modelling ($>75$ observations as per \mbox{%DIFAUXCMD
\cite{edwards_point_2023,solymos_evaluating_2018,buckland_introduction_2001}}\hskip0pt%DIFAUXCMD
). 
The each of the trees represents a posterior sample from a phylogenetic relationship built by \mbox{%DIFAUXCMD
\citet{jetz_global_2012}}\hskip0pt%DIFAUXCMD
; that is, no one tree is the ``correct" phylogenetic relationship, but instead represents a point from a posterior distribution of trees.
We also included the following bird species with insufficient removal and/or distance data: Lesser Prairie-chicken, Spotted Owl, Bicknell's Thrush, LeConte's Thrasher, Harris's Sparrow, Tricolored Blackbird, and Kirtland's Warbler. 
These species are all on Partners in Flight's watchlist, indicating their conservation importance \mbox{%DIFAUXCMD
...
}\DIFaddend We did not include Woodhouse's Scrub-jay (\textit{Aphelocoma woodhouseii}), Pacific Wren (\textit{Troglodytes pacificus}), or Sagebrush Sparrow (\textit{Artemisiospiza nevadensis}) in the analysis, because these species were not available in the phylogenies from \citet{jetz_global_2012}.
\DIFaddbegin \DIFadd{This resulted in 2000 pseudo-posterior trees that contained 323 species.
}

\par \DIFadd{We generated a phylogenetic correlation matrix $\mathbf{C}$ using the same methods outlined in the Appendix of \mbox{%DIFAUXCMD
\citet{solymos_phylogeny_2018} }\hskip0pt%DIFAUXCMD
using the R package ``ape" \mbox{%DIFAUXCMD
\citep{paradis_ape_2019}}\hskip0pt%DIFAUXCMD
. 
...
\subsubsection{\DIFadd{Species Traits}}
\par \DIFadd{Species traits (song pitch, migratory vs resident, open vs. closed habitat) were obtained from Birds of the World accounts for all species considered in this study \mbox{%DIFAUXCMD
\citep{billerman_birds_2022}}\hskip0pt%DIFAUXCMD
. 
Species were classified as ``closed habitat" if their primary habitat was forests, and ``open habitat" if their primary habitat was grasslands or shrublands.
Species with specific areas of their range that are only for breeding (i.e., not year-round throughout their range) were classified as ``migratory".
...
Body mass for each bird species was obtained from the Elton traits dataset if available \mbox{%DIFAUXCMD
\citep{wilman_eltontraits_2014}}\hskip0pt%DIFAUXCMD
, and otherwise through the Birds of the World account \mbox{%DIFAUXCMD
\citep{billerman_birds_2022} }\hskip0pt%DIFAUXCMD
if not.
}


\subsection{\DIFadd{Model Assessment}}
\subsubsection{\DIFadd{Objective 1: Single-Species vs. Multi-Species Estimates}}
\par \DIFadd{To test our prediction that there would be little change between estimates of species with sufficient sizes, we ran the Bayesian hierarchical QPAD Removal and Distance models on the set of species for which there was sufficient data.
}\DIFaddend Rufous Hummingbird (\textit{Selasphorus rufus}) was dropped from the distance modelling due to issues with divergent transitions \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
That is, for each of the ten cross-validation runs, we held out all data for 10\% of the species for both the multi-species and single-species models\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, effectively creating a "leave-species-out cross validation".
}
...
\par \DIFaddend To determine which species would be left out in each fold of the distance model \DIFaddbegin \DIFadd{cross validation}\DIFaddend , we randomly assigned a number between 1 and 10 (drawn from a uniform distribution) to each species \DIFdelbegin \DIFdel{within a migratory resident $\times$ habitat preference , }\DIFdelend \DIFaddbegin \DIFadd{of each combination of migratory status group and habitat preference group, e.g., migrant forest birds, resident grassland birds, }\DIFaddend which ensured that there would be a good balance of species left out for each group of species.
...
\DIFaddbegin \DIFadd{For removal modelling, we experienced no issues related to initialization, sampling, or convergence. 
...
\citet{aubry_bicknells_2018}}\hskip0pt%DIFAUXCMD
, however, estimate detection probabilities of $\geq$ 0.74 and $\leq$ 0.88, and so even our highest potential value of detectability falls short of their estimated range.
However, we note that \mbox{%DIFAUXCMD
\citet{aubry_bicknells_2018} }\hskip0pt%DIFAUXCMD
are using data that specifically target Bicknell's Thrush in their range, and they are using surveys conducted at peak Bicknell's Thrush breeding time, and so it is likely that detectability will be high in that specific case.
}%DIFDELCMD < 

%DIFDELCMD < \par %%%
\DIFdelend For Kirtland’s Warbler, using the predicted cue rate of 0.29, predicted EDR of 75.3 m, and an example survey design of 5 minutes recording all birds within 100 m, we estimate a detectability of 0.36.
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{van_dyke_comparative_2022} }\hskip0pt%DIFAUXCMD
found estimated detection probabilities for Kirtland’s Warbler that ranged from effectively 0, through to a detection probability of 0.73.
In that study, they compared estimates of detectability among different jack pine (}\textit{\DIFdel{Pinus banksiana}}%DIFAUXCMD
\DIFdel{) stands and red pine (}\textit{\DIFdel{Pinus resinosa}}%DIFAUXCMD
\DIFdel{) stands, where the detectability was higher in jack pine stands than the red pine stands.
}\DIFdelend 
...
\par Overall, the multi-species models generated very similar estimates of cue rate and EDR as single-species models, for species where we had sufficient data points \DIFaddbegin \DIFadd{(Figure \ref{fig:1vs1})}\DIFaddend .
...
In this case, a multi-species model \DIFaddbegin \DIFadd{that is based on only a few traits }\DIFaddend brings the estimates down to a more realistic value for the species based on other similar species.
Of course, this may neglect species-specific idiosyncrasies in detectability (but see subsection \ref{discussion-nodata}).

\par We did not see a large difference in precision of estimates between the single-species and multi-species model, except for a slight difference in cue rate estimates for species with sample sizes less than \DIFdelbegin \DIFdel{1000.
}\DIFdelend \DIFaddbegin \DIFadd{1000 (Figure \ref{fig:sd}).
}\DIFaddend However, we note that our baseline single-species model was a Bayesian implementation of the QPAD models described in \citet{solymos_calibrating_2013}, and we used weakly informative $N(0,1)$ priors on the parameters for cue rate and EDR.
...
\DIFaddbegin \par \DIFadd{We note that while this only demonstrates internal consistency because we did not test compared to a simulation study, we direct readers to \mbox{%DIFAUXCMD
\citet{solymos_calibrating_2013, solymos_evaluating_2018, solymos_lessons_2020} }\hskip0pt%DIFAUXCMD
for discussions regarding goodness-of-fit of the QPAD methodology in general.
...
\DIFaddend \subsection{Predicting Detectability for Data-sparse Species}
...
With this in mind, while studies have shown that traits are useful for explaining variation in EDR \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{solymos_phylogeny_2018, johnston_species_2014}}\hskip0pt%DIFAUXCMD
}\DIFaddend , traits alone do not appear to be sufficient in predicting EDRs in similar birds, because traits cannot account for species-specific variation in EDR. 
...
\par Given the results of the k-fold cross-validation \DIFaddbegin \DIFadd{(Figure \ref{fig:cv})}\DIFaddend , we were comfortable with applying this model to species where we had \DIFdelbegin \DIFdel{only a few data points }\DIFdelend \DIFaddbegin \DIFadd{fewer data points than what one would typically use for removal and distance modelling}\DIFaddend , such as Lesser Prairie-chicken, Spotted Owl, LeConte's Thrasher, Harris's Sparrow, and Tricolored Blackbird \DIFaddbegin \DIFadd{(Figure \ref{fig:predictions})}\DIFaddend .
...
\par We caution, however, against the use of these models for species with no data points, given the result of the leave-species-out cross validation \DIFaddbegin \DIFadd{(Figure \ref{fig:species_cv} and \ref{fig:distance_cv_traits})}\DIFaddend .
...
\DIFaddbegin \DIFadd{For Bicknell's thrush, \mbox{%DIFAUXCMD
\citet{aubry_bicknells_2018}}\hskip0pt%DIFAUXCMD
, estimate detection probabilities of $\geq$ 0.74 and $\leq$ 0.88, and so even our highest potential value of detectability falls short of their estimated range.
}\DIFaddend However, we note that \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{aubry_bicknells_2018} }\hskip0pt%DIFAUXCMD
are using data that specifically target Bicknell's Thrush in their range, and they are using surveys conducted at peak Bicknell's Thrush breeding time, and so it is likely that detectability will be high in that specific case.
\mbox{%DIFAUXCMD
\citet{van_dyke_comparative_2022} }\hskip0pt%DIFAUXCMD
found estimated detection probabilities for Kirtland’s Warbler that ranged from effectively 0, through to a detection probability of 0.73.
In that study, they compared estimates of detectability among different jack pine (}\textit{\DIFadd{Pinus banksiana}}\DIFadd{) stands and red pine (}\textit{\DIFadd{Pinus resinosa}}\DIFadd{) stands, where the detectability was higher in jack pine stands than the red pine stands.
For }\DIFaddend species with low populations in specific habitats such as these species, highly-targeted surveys are often used such that reported detectability rates may be higher than for ``normal" (i.e., more broadly-sampled) species.
...
This approach seemed to work as the initial values were no longer rejected\DIFdelbegin \DIFdel{, but we caution that this could very well be an ad hoc solution to an underlying issue with the model itself}\DIFdelend .
...
\DIFdelbegin \DIFdel{We demonstrate some of the odd sampling of the posterior for Rufous Hummingbird in Supplemental Materials B.
}\DIFdelend One potential explanation is that Rufous Hummingbird had the smallest EDR of all the species in NA-POPS, and so there could be an issue with numerical stability of sampling very low values, due to the exponentiation of a quotient in the distance modelling likelihood.
...
\DIFdelbegin %DIFDELCMD < \par %%%
\DIFdel{For removal modelling, we experienced no issues related to initialization, sampling, or convergence. 
The model finished within approximately 24 hours, and all parameters had R-hat values of 1, or within 0.001 of it.
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Improvements to this model}
...
\DIFaddbegin \DIFadd{In particular, even though we considered a constant cue rate (i.e., one that does not change through the year), it is well known that cue rate changes with time of year and time of day \mbox{%DIFAUXCMD
\citep{solymos_evaluating_2018, edwards_point_2023}}\hskip0pt%DIFAUXCMD
.
Although we positioned this study to differ from other multi-species models such as \mbox{%DIFAUXCMD
\citet{gilbert_multispecies_2024} }\hskip0pt%DIFAUXCMD
by sharing information among the observational process, including information on the ecological processes that drive abundance and distribution could be crucial to improving these models. 
}\DIFaddend This is especially true for the types of species we are interested in generating predictions for, i.e., rare and/or undersampled species.
